...
// Real, SDK-exported invariants (@raydium-io/raydium-sdk-v2's MIN_TICK/MAX_TICK)
// — not guessed, same underlying 1.0001^tick math as Orca's tick bounds.
...
interface RaydiumAddLiquidityPanelProps {
  hookAddress: string;
  riskLevel: string;
...
export function RaydiumAddLiquidityPanel({ hookAddress, riskLevel, hookScore }: RaydiumAddLiquidityPanelProps) {
  ...
  useEffect(() => {
    ...
    api.raydiumLp.quote({ poolId: pool.poolId, tickLower: fullRange.tickLower, tickUpper: fullRange.tickUpper })
      .then((q) => {
  ...
  const handleMint = useCallback(async () => {
    ...
    setBuilding(true);
    try {
      const built = await api.raydiumLp.build({
        poolId: pool.poolId,
        tickLower: ticks.tickLower,
        tickUpper: ticks.tickUpper,
